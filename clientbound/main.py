...
@public
class CafeUnknownPacket2(ClientboundPacket):
    """Unknown Cafe packet.

    Observed with 2 bytes of data.
    """
...
@public
class CafeListPacket(ClientboundPacket):
    """List of Cafe topics/posts.

    Contains the list of topics displayed in the Cafe.
    """
    ...
    @property
    def titles(self):
        """Extract topic titles from the payload.
        ...
        """
        return cafe.extract_topic_titles_from_list_payload(bytes(self.data))

    @property
    def topics(self):
        """Extract topics with IDs and titles from the payload.

        Returns
        -------
        list[dict]
            List of dictionaries with 'id', 'title', and 'author' keys.
        """
        return cafe.extract_topics_from_list_payload(bytes(self.data))

    def __repr__(self):
        topics = self.topics
        return f"CafeListPacket(topics={topics!r})"

@public
class CafeTopicPostsPacket(ClientboundPacket):
    """Posts within a specific Cafe topic.

    Sent when opening a topic to view its posts/replies.
    """
    ...
    @property
    def messages(self):
        """Extract messages from the topic payload.
        ...
        """
        return cafe.extract_messages_from_topic_payload(bytes(self.data))

    @property
    def posts(self):
        """Extract posts with IDs, usernames, and messages from the payload.

        Returns
        -------
        list[dict]
            List of dictionaries with 'post_id', 'username', and 'message' keys.
        """
        return cafe.extract_posts_from_topic_payload(bytes(self.data))

    def __repr__(self):
        posts = self.posts
        return f"CafeTopicPostsPacket(posts={posts!r})"

@public
class CafeNewPostNotificationPacket(ClientboundPacket):
    """Notification when a new Cafe post is created.

    Observed payload: [topic_id:int][username:String][topic_message_id:int].
    """
    ...
    topic_id: types.Int
    username: types.String
    topic_message_id: types.Int

    def __repr__(self):
        return (
            "CafeNewPostNotificationPacket("
            f"topic_id={self.topic_id!r}, username={self.username!r}, post_id={self.topic_message_id!r}"
            ")"
        )

@public
class CafeUnknownPacket(ClientboundPacket):
    """Unknown Cafe packet.

    Observed with 1 byte of data after opening Cafe.
    """
