...
@public
class CafeRefreshPacket(ServerboundPacket):
    """Refresh Cafe listing/categories.

    Observed as an empty-body request with a fingerprint.
    """

    id = (30, 40)

@public
class CafeCreateTopicPacket(ServerboundPacket):
...
@public
class CafeOpenTopicPacket(ServerboundPacket):
    """Open/view a specific Cafe topic.

    Sent when clicking on a topic in the Cafe to view its posts.
    """
...
@public
class CafeTopicPacket(ServerboundPacket):
    """Select/filter Cafe topics.

    Sent when selecting a topic filter in the Cafe.
    """

    id = (30, 45)

    topic_id:    types.UnsignedByte

@public
class CafeReplyTopicPacket(ServerboundPacket):
    id = (30, 43)
...
@public
class CafeLikePostPacket(ServerboundPacket):
    """Like or unlike a Cafe post.

    Sent when clicking the like button on a post in the Cafe.
    """
...
@public
class CafeReportMessagePacket(ServerboundPacket):
    id = (149, 4)
...
@public
class PlayerReportPacket(ServerboundPacket):
...
@public
class SendLetterPacket(ServerboundPacket):
    """
    Send a letter (mail) to a player.

    This packet is used when sending a letter with:
      - a drawing
      - OR no drawing at all (empty canvas)

    Payload format:
      - recipient (String)          -> target username#tag
      - tool / mode (u8)            -> drawing tool / mode
      - drawing_data (Raw bytes)    -> drawing strokes OR minimal footer
    """
